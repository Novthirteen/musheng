...
AjaxControlToolkit.Seadragon.Viewport = function(containerSize, contentSize, config) {
	this.zoomPoint = null;
	this.config = config;
	...
	this._homeBounds = new AjaxControlToolkit.Seadragon.Rect(0, 0, 1, this._contentHeight);
	this.goHome(true);
	this.update();
}
AjaxControlToolkit.Seadragon.Viewport.prototype = {
	...
	getContainerSize: function() {
		return new AjaxControlToolkit.Seadragon.Point(this._containerSize.x, this._containerSize.y);
	},
	...
	getBounds: function(current) {
		...
		return new AjaxControlToolkit.Seadragon.Rect(center.x - width / 2.0, center.y - height / 2.0,
            width, height);
	},

	getCenter: function(current) {
		var centerCurrent = new AjaxControlToolkit.Seadragon.Point(this._centerSpringX.getCurrent(),
                this._centerSpringY.getCurrent());
		var centerTarget = new AjaxControlToolkit.Seadragon.Point(this._centerSpringX.getTarget(),
		...
		var width = 1.0 / zoom;
		var height = width / this.getAspectRatio();
		var bounds = new AjaxControlToolkit.Seadragon.Rect(centerCurrent.x - width / 2.0,
	...
	fitBounds: function(bounds, immediately) {
		...
		var newBounds = new AjaxControlToolkit.Seadragon.Rect(bounds.x, bounds.y, bounds.width, bounds.height);
	...
	panBy: function(delta, immediately) {
		...
		var center = new AjaxControlToolkit.Seadragon.Point(this._centerSpringX.getTarget(),
                this._centerSpringY.getTarget());
	...
	zoomTo: function(zoom, refPoint, immediately) {
		...
		this.zoomPoint = refPoint instanceof AjaxControlToolkit.Seadragon.Point ? refPoint : null;
	},
	...
	resize: function(newContainerSize, maintain) {
		...
		this._containerSize = new AjaxControlToolkit.Seadragon.Point(newContainerSize.x, newContainerSize.y);
...
AjaxControlToolkit.Seadragon.Viewport.registerClass('AjaxControlToolkit.Seadragon.Viewport', null, Sys.IDisposable);
