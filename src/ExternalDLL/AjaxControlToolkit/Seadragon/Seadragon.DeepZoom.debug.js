...
AjaxControlToolkit.Seadragon.DziError = function(message) {
    AjaxControlToolkit.Seadragon.DziError.initializeBase(this, [message]);
    this.message = message;
}
AjaxControlToolkit.Seadragon.DziError.registerClass('AjaxControlToolkit.Seadragon.DziError', Error);

AjaxControlToolkit.Seadragon.DziTileSource = function(width, height, tileSize, tileOverlap, tilesUrl, fileFormat, displayRects) {
    AjaxControlToolkit.Seadragon.DziTileSource.initializeBase(this, [width, height, tileSize, tileOverlap, null, null]);
    this._levelRects = {};
...
AjaxControlToolkit.Seadragon.DziTileSource.prototype = {
    _init: function() {
...
AjaxControlToolkit.Seadragon.DziTileSource.registerClass('AjaxControlToolkit.Seadragon.DziTileSource', AjaxControlToolkit.Seadragon.TileSource);
AjaxControlToolkit.Seadragon._DziTileSourceHelper = function() {

}
AjaxControlToolkit.Seadragon._DziTileSourceHelper.prototype = {
    createFromXml: function(xmlUrl, xmlString, callback) {
        ...
        if (!xmlUrl) {
            ...
            throw new AjaxControlToolkit.Seadragon.DziError(error);
        }
    ...
    processResponse: function(xhr, tilesUrl) {
        if (!xhr) {
            throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Security"));
        } else if (xhr.status !== 200 && xhr.status !== 0) {
            // chrome has bug where it sends "OK" for 404
            var status = xhr.status;
            var statusText = (status == 404) ? "Not Found" : xhr.statusText;
            throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Status", status, statusText));
        }
    ...
    processXml: function(xmlDoc, tilesUrl) {
        if (!xmlDoc || !xmlDoc.documentElement) {
            throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Xml"));
        }
        ...
        if (rootName == "Image") {
            try {
                ...
                throw (e instanceof AjaxControlToolkit.Seadragon.DziError) ? e : new AjaxControlToolkit.Seadragon.DziError(defMsg);
            }
        } else if (rootName == "Collection") {
            throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Dzc"));
        } else if (rootName == "Error") {
            return this.processError(root);
        }

        throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Dzi"));
    },
    ...
    processDzi: function(imageNode, tilesUrl) {
        ...
        if (!Seadragon.Utils.imageFormatSupported(fileFormat)) {
            throw new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.ImageFormat",
        ...
        return new AjaxControlToolkit.Seadragon.DziTileSource(width, height, tileSize, tileOverlap,
    ...
    processError: function(errorNode) {
        ...
        throw new AjaxControlToolkit.Seadragon.DziError(message);
    },
    getError: function(e) {
        if (!(e instanceof DziError)) {
            ...
            e = new AjaxControlToolkit.Seadragon.DziError(Seadragon.Strings.getString("Errors.Unknown"));
        }

    }
}
//Static objects
AjaxControlToolkit.Seadragon.DziTileSourceHelper = new AjaxControlToolkit.Seadragon._DziTileSourceHelper();