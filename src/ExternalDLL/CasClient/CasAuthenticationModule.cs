...
using com.Sconit.CasClient.Configuration;
using com.Sconit.CasClient.Utils;
using com.Sconit.CasClient.Security;
using com.Sconit.CasClient.Session;
using com.Sconit.CasClient.Validation;
using System.Web.SessionState;
using log4net;

namespace com.Sconit.CasClient
{
  /// <summary>
    ...
    void OnAcquireRequestState(object sender, EventArgs args)
    {
      ...
      if (this.SingleSignOut) {
        ...
        if (CommonUtils.IsNotBlank(ticket)) {
          if (log.IsDebugEnabled) {
            log.Debug(string.Format("{0}: updating single sign out using ticket={1}",
      ...
      if (contextUserPrincipal != null) {
        ...
        if (typeof(CasPrincipal) == contextUserPrincipal.GetType()) {
          ...
          session[AbstractCasModule.CONST_CAS_PRINCIPAL] = contextUserPrincipal;
        } else {
          ...
          if (CommonUtils.CAS_AUTH_TYPE.Equals(contextUserPrincipal.Identity.AuthenticationType)) {
            ...
            Object storedPrincipalObj = session[AbstractCasModule.CONST_CAS_PRINCIPAL];
            if (storedPrincipalObj != null) {
              if (log.IsDebugEnabled) {
                log.Debug(string.Format("{0}:starting session Principal={1}",
                  CommonUtils.MethodName, 
                  DebugUtils.IPrincipalToString((IPrincipal)contextUserPrincipal)));
              }
              try {
                ICasPrincipal storedPrincipal = 
                (ICasPrincipal) session[AbstractCasModule.CONST_CAS_PRINCIPAL];
                if (log.IsDebugEnabled) {
            ...
            if (contextUserPrincipal == null) {
              ...
              session.Remove(AbstractCasModule.CONST_CAS_PRINCIPAL);
