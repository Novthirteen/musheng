...
namespace com.Sconit.CasClient
{
  ...
  { 
    ...
    protected bool IsCasProtected(HttpApplication application)
    {
      ...
      if (log.IsDebugEnabled) {
        log.Debug(string.Format("{0}:Checking {1} using secureUriRegex {2} --> {3}",
    ...
    public void Application_AcquireRequestState(object sender, EventArgs e)
    {
      ...
      try
      {
          ...
          if (session[CONST_CAS_PRINCIPAL] != null)
          {
              ICasPrincipal sessionPrincipal = (ICasPrincipal)session[CONST_CAS_PRINCIPAL];
              if (log.IsDebugEnabled)
              {
          ...
          if (log.IsDebugEnabled)
          {
              log.Debug("May be Url is request the WebResource or Other URL");
          }
      }
      if (log.IsDebugEnabled) {
    ...
    private void AuthenticateStep(HttpApplication application, EventArgs e)
    {
      ...
      ICasPrincipal sessionPrincipal = session != null ?
    ...
    private void ValidateStep(HttpApplication application, EventArgs e)
    {
      ...
      if (CommonUtils.IsNotBlank(ticket)) {
        ...
        try {
          ...
          session[CONST_CAS_PRINCIPAL] = principal;
