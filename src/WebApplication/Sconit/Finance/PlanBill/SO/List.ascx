    ...
    <div class="GridView">
        <asp:GridView ID="GV_List" runat="server" AllowPaging="False" DataKeyNames="Id" AllowSorting="False"
            ...
            <Columns>
                ...
                <asp:BoundField DataField="ExternalReceiptNo" HeaderText="${MasterData.PlannedBill.ExtReceiptNo}"
                    SortExpression="ExternalReceiptNo" />
                <asp:TemplateField HeaderText=" ${MasterData.PlannedBill.Customer.Name}" SortExpression="BillAddress.Party.Name">
                    <ItemTemplate>
                        <asp:Label ID="lblCustomer" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "BillAddress.Party.Code")%>'
                ...
                <asp:TemplateField HeaderText="${MasterData.PlannedBill.CreateDate}" SortExpression="CreateDate">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="${MasterData.PlannedBill.CurrentActingQty}">
                    <ItemTemplate>
                        <asp:TextBox ID="tbCurrentActingQty" runat="server" onmouseup="if(!readOnly)select();"
                            Width="50" Text='<%# Bind("CurrentActingQty","{0:0.########}") %>' />
