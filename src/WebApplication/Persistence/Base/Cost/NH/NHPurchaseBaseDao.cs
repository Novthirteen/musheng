using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.Entity.Cost;

//TODO: Add other using statmens here.

namespace com.Sconit.Persistence.Cost.NH
{
    public class NHPurchaseBaseDao : NHDaoBase, IPurchaseBaseDao
    {
        public NHPurchaseBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreatePurchase(Purchase entity)
        {
            Create(entity);
        }

        public virtual IList<Purchase> GetAllPurchase()
        {
            return FindAll<Purchase>();
        }

        public virtual Purchase LoadPurchase(Int32 id)
        {
            return FindById<Purchase>(id);
        }

        public virtual void UpdatePurchase(Purchase entity)
        {
            Update(entity);
        }

        public virtual void DeletePurchase(Int32 id)
        {
            string hql = @"from Purchase entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeletePurchase(Purchase entity)
        {
            Delete(entity);
        }

        public virtual void DeletePurchase(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from Purchase entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeletePurchase(IList<Purchase> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (Purchase entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeletePurchase(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
