using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.Entity.View;

//TODO: Add other using statmens here.

namespace com.Sconit.Persistence.View.NH
{
    public class NHOrderDetailViewBaseDao : NHDaoBase, IOrderDetailViewBaseDao
    {
        public NHOrderDetailViewBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateOrderDetailView(OrderDetailView entity)
        {
            Create(entity);
        }

        public virtual IList<OrderDetailView> GetAllOrderDetailView()
        {
            return FindAll<OrderDetailView>();
        }

        public virtual OrderDetailView LoadOrderDetailView(Int32 id)
        {
            return FindById<OrderDetailView>(id);
        }

        public virtual void UpdateOrderDetailView(OrderDetailView entity)
        {
            Update(entity);
        }

        public virtual void DeleteOrderDetailView(Int32 id)
        {
            string hql = @"from OrderDetailView entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeleteOrderDetailView(OrderDetailView entity)
        {
            Delete(entity);
        }

        public virtual void DeleteOrderDetailView(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from OrderDetailView entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteOrderDetailView(IList<OrderDetailView> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (OrderDetailView entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeleteOrderDetailView(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
