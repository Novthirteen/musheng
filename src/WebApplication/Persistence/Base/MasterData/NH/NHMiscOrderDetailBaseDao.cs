using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.Entity.MasterData;

//TODO: Add other using statmens here.

namespace com.Sconit.Persistence.MasterData.NH
{
    public class NHMiscOrderDetailBaseDao : NHDaoBase, IMiscOrderDetailBaseDao
    {
        public NHMiscOrderDetailBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateMiscOrderDetail(MiscOrderDetail entity)
        {
            Create(entity);
        }

        public virtual IList<MiscOrderDetail> GetAllMiscOrderDetail()
        {
            return FindAll<MiscOrderDetail>();
        }

        public virtual MiscOrderDetail LoadMiscOrderDetail(Int32 id)
        {
            return FindById<MiscOrderDetail>(id);
        }

        public virtual void UpdateMiscOrderDetail(MiscOrderDetail entity)
        {
            Update(entity);
        }

        public virtual void DeleteMiscOrderDetail(Int32 id)
        {
            string hql = @"from MiscOrderDetail entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeleteMiscOrderDetail(MiscOrderDetail entity)
        {
            Delete(entity);
        }

        public virtual void DeleteMiscOrderDetail(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from MiscOrderDetail entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteMiscOrderDetail(IList<MiscOrderDetail> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (MiscOrderDetail entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeleteMiscOrderDetail(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
