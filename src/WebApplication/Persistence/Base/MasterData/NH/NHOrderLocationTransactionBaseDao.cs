using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.Entity.MasterData;

//TODO: Add other using statmens here.

namespace com.Sconit.Persistence.MasterData.NH
{
    public class NHOrderLocationTransactionBaseDao : NHDaoBase, IOrderLocationTransactionBaseDao
    {
        public NHOrderLocationTransactionBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateOrderLocationTransaction(OrderLocationTransaction entity)
        {
            Create(entity);
        }

        public virtual IList<OrderLocationTransaction> GetAllOrderLocationTransaction()
        {
            return FindAll<OrderLocationTransaction>();
        }

        public virtual OrderLocationTransaction LoadOrderLocationTransaction(Int32 id)
        {
            return FindById<OrderLocationTransaction>(id);
        }

        public virtual void UpdateOrderLocationTransaction(OrderLocationTransaction entity)
        {
            Update(entity);
        }

        public virtual void DeleteOrderLocationTransaction(Int32 id)
        {
            string hql = @"from OrderLocationTransaction entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeleteOrderLocationTransaction(OrderLocationTransaction entity)
        {
            Delete(entity);
        }

        public virtual void DeleteOrderLocationTransaction(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from OrderLocationTransaction entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteOrderLocationTransaction(IList<OrderLocationTransaction> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (OrderLocationTransaction entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeleteOrderLocationTransaction(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
