...
namespace com.Sconit.Service.Report
{
     ...
     * 
     */
    public abstract class RepTemplate3 : ReportBaseMgr
    {
        //明细部分的行数
        ...
        public override bool FillValues(String templateFileName, IList<object> list)
        {
            ...
            this.rowCount = pageDetailRowCount / this.fieldsCount + bottomRowCount;
        ...
        {
            return (rowIndex == (this.resultCount - 1) || ((rowIndex + 1) % (this.pageDetailRowCount / this.fieldsCount) == 0) || this.isLastPage(rowTotal));
        }
         ...
         * 
         */
        protected virtual int PageCount(int resultCount)
        {
        ...
        */
        protected override int GetRowIndexAbsolute(int pageIndex, int rowIndexRelative)
        {
            ...
            if (pageIndex > 1)
            {
                rowIndexAbsolute = this.headRowCount + this.pageDetailRowCount / this.fieldsCount + this.bottomRowCount + (this.pageDetailRowCount / this.fieldsCount + this.bottomRowCount) * (pageIndex - 2) + rowIndexRelative;
                //(头行数 + 列头 + 明细 +尾巴) (页数-1)
