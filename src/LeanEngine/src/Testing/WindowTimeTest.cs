...
namespace Testing
{
    ...
    public class WindowTimeTest
    {
        ...
        public void Flow2_Test4()
        {
            ...
            if (expected3 <= DateTime.Now.AddHours(1))
            {
                expected1 = expected1.AddHours(2);
                expected2 = expected2.AddHours(2);
                expected3 = expected3.AddHours(2);
            }
        ...
        public void Flow2_Test5()
        {
            ...
            flow2.FlowStrategy.SatWinTime = winTimes;
            flow2.FlowStrategy.SunWinTime = winTimes;
            flow2.FlowStrategy.LeadTime = 72;
            flow2.FlowStrategy.EmLeadTime = 48;
            flow2.WindowTime = DateTime.Parse("2012-9-30 18:00");
            flow2.OrderTime = DateTime.Now;

            kb.ProcessTime(flow2);
            DateTime expected = DateTime.Parse("2012-9-30 20:00");
            Assert.AreEqual(expected, flow2.NextWindowTime.Value);
        }
    }
}
