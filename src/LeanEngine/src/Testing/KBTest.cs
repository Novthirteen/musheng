...
namespace Testing
{
    ...
    public class KBTest
    {
        ...
        //新功能支持:2010-8-20,Kanban多库位拉动
        [Test]
        public void Test_MultiLoc()
        {
            string loc2 = "FG";
            
            itemFlow.DemandSources = new List<string>();
            itemFlow.DemandSources.Add(locTo);
            itemFlow.DemandSources.Add(loc2);

            InvBalance invBalance2 = new InvBalance();
            invBalance2.Item = item;
            invBalance2.Loc = loc2;
            invBalance2.Qty = 10;
            invBalances.Add(invBalance2);

            engine.TellMeDemands(container);
            Assert.AreEqual(20, itemFlow.ReqQty);
        }
    }
}
